...
let loc=useLocation();
  ...
  ;let get=await fetch("http://localhost:4800/login",{credentials:'include'});if(!get.ok){let conv=await get.json();throw new Error(conv||'error in login')};return await get.json() },retry:false ,refetchOnWindowFocus:false,refetchInterval:false,enabled:sign_property.status||loc.pathname.includes("completed")?false:true});
...
return (<div ref={ref} className={`border-3 flex absolute top-0 justify-center duration-1000 items-center text-white border-yellow-900/50 bg-yellow-900/25 z-10 w-100 h-[100px] z-3  mt-0 ml-10 origin-top `}>Loading </div>)
